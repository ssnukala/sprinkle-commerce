...
namespace UserFrosting\Sprinkle\Commerce\Services\Processors;

use UserFrosting\Sprinkle\Commerce\Services\PaymentProcessorInterface;
 ...
 */
class GooglePayProcessor implements PaymentProcessorInterface
{
    /**
     * {@inheritDoc}
     */
    public function process(Model $payment, array $data): array
    {
        try {
            ...
            if (!isset($data['payment_token'])) {
                return [
                    'success' => false,
                    'error' => 'Google Pay token is required',
                ];
            }

            // Store payment details
            $payment->paymentDetails()->create([
                'detail_type' => 'google_pay_token',
                'key' => 'payment_token',
