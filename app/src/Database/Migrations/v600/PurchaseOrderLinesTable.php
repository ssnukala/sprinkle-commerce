 ...
 */
class PurchaseOrderLinesTable extends Migration
{
    /**
     * {@inheritDoc}
     */
    ...
    {
        if (!$this->schema->hasTable('or_purchase_order_lines')) {
            $this->schema->create('or_purchase_order_lines', function (Blueprint $table) {
                $table->increments('id');
    ...
    {
        $this->schema->drop('or_purchase_order_lines');
    }
}
